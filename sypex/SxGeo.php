...
class SxGeo {
	...
	public int|bool $batch_mode = false;
	public int|bool $memory_mode = false;
	...
	public function get_num($ip): float|false|int {
	...
	protected function parseCity($seek, $full = false): false|array {
		if (!$this->pack) {
			return false;
		}
		$only_country = false;
	...
	public function getCity($ip): false|array {
		$seek = $this->get_num($ip);
		return $seek ? $this->parseCity($seek) : false;
	}

	public function getCityFull($ip): false|array {
		$seek = $this->get_num($ip);
